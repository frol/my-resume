...
\cventry{June\,2012 -- today}{Software Engineer}{Data Mining and Predictive Analytics Software company}{}{}{%
\begin{itemize}%
    \item Designed and developed a distributed data mining (BigData) project \\
        \textsc{\color{gray} -- \mbox{Python}, \mbox{Yarn} \mbox{Hadoop}, \mbox{PySpark}, \mbox{Python C-extensions}, \mbox{PyInvoke}, \mbox{Anaconda}}
    \item Designed and developed a data mining SaaS with a distributed backend in the cloud \\
        \textsc{\color{gray} -- \mbox{Python}, \mbox{Django}, \mbox{PostgreSQL}, \mbox{Redis}, \mbox{AWS}, \mbox{HTML/CSS/JS}}
    ...
    \item Set up Salford Systems IT infrastructures \\
        \textsc{\color{gray} -- \mbox{Linux}, \mbox{Docker}, \mbox{Hadoop}, \mbox{FreeIPA}, \mbox{CFEngine}, \mbox{NGINX}, \mbox{Jenkins}, \mbox{Redmine}, \mbox{PyInvoke}}
\end{itemize}}
...
Developed a food delivery web-service for restaurant chains ``Mafia'' (mafia.ua) and ``Yakitoria'' (yaki.kh.ua) \\ %
...
\cvitem{Programming Languages}{\small\textsc{%
        Python $\circ$ Bash $\circ$ C++ $\circ$ C $\circ$ JavaScript $\circ$ SQL $\circ$
        HTML $\circ$ {\LaTeX} $\circ$ Delphi/Pascal
}}
\cvitem{Frameworks and Tools}{\small\textsc{%
        Django $\circ$ Flask $\circ$ Celery $\circ$ PySpark $\circ$ Apache Spark $\circ$
        Hadoop YARN $\circ$ ReactJS $\circ$ jQuery $\circ$ AngularJS $\circ$ VIM $\circ$
        Git $\circ$ SVN $\circ$ Mercurial $\circ$ NGINX $\circ$ Apache $\circ$ PyInvoke $\circ$
        StrongSWAN $\circ$ Jenkins $\circ$ Anaconda $\circ$ Qt
}}
\cvitem{Databases}{\small\textsc{%
        MySQL $\circ$ PostgreSQL $\circ$ SQLite $\circ$ MongoDB $\circ$ RabbitMQ $\circ$
        Redis $\circ$ Memcache
}}
